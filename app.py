...
from models.llm import get_openai_model
from utils.web_search import web_search
from utils.rag_pipeline import create_vector_store, retrieve_context

def get_chat_response(chat_model, messages, system_prompt, use_web_search=False):
    ...
    try:
        ...
        if use_web_search:
            web_results = web_search(last_user_message)

            system_prompt += f"""

            Use the following real-time web information to answer the user query:

            {web_results}

            If the web results are not useful, do not include it in your response.
            But Mention "I am unable to find your relevant query on the internet, try rephrasing !
            """
        #--- RAG
        ...
        if context:
            ...
            If the context does not help answer the user question, answer normally.
            """
        ...
        for msg in messages:
            ...
            elif msg["role"] == "assitant": ## made changes here.
...
def chat_page():
    ...
    st.title("🤖 Smart Research Assistant Chatbot")
    ...
    # Save file only once
        if "doc_uploaded" not in st.session_state:
            ...
            with st.spinner("Generating embeddings and indexing document... Please wait"):
                create_vector_store("uploaded_doc.pdf")
    ...
    if response_mode == "Concise":
        system_prompt = """
        You are a helpful AI assistant. Respond to the user query in a concise way.
        Keep your response short, clear, and summarized not more than 2–3 sentences if possible.
        """
    else:
        system_prompt = """
        You are a helpful AI assistant. Respond in a detailed and a very explanatory way possible.
        Provide deeper explanations, examples, and structured answers based on user query.
        """
    ...
    if prompt := st.chat_input("Type your message here..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Getting response..."):
                response = get_chat_response(chat_model, st.session_state.messages, system_prompt, use_web_search)
                st.markdown(response)
        
        # Add bot response to chat history
        st.session_state.messages.append({"role": "assistant", "content": response})
    # else:
    #     st.info("🔧 Welcome Researcher!!!")
